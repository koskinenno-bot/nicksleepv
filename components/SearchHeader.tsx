...
const SearchHeader: React.FC<Props> = ({ onSearch, loadingState, compact }) => {
  ...
  return (
    <header className={`sticky top-0 z-30 w-full border-b border-nomad-800 bg-nomad-950/80 backdrop-blur-md transition-all duration-500 ${compact ? 'py-4' : 'py-16'}`}>
      ...
      <div className={`max-w-6xl mx-auto px-6 flex flex-col items-center relative z-10 ${compact ? 'md:flex-row md:justify-between gap-4' : ''}`}>
        {!compact && (
          <>
        ...
        <form onSubmit={handleSubmit} className={`relative w-full group ${compact ? 'max-w-md' : 'max-w-lg'}`}>
          ...
          <input
            ...
            className={`relative w-full bg-nomad-800/80 border border-nomad-600 text-nomad-100 rounded-full focus:outline-none focus:ring-2 focus:ring-yellow-500/50 focus:border-yellow-600 placeholder-nomad-500 transition-all shadow-2xl backdrop-blur-sm ${compact ? 'py-2 px-6 text-sm' : 'py-4 px-8 text-lg'}`}
            disabled={isLoading}
          />
          <button
            type="submit"
            disabled={isLoading}
            className={`absolute right-1.5 top-1.5 bottom-1.5 bg-nomad-700 hover:bg-yellow-600 text-nomad-200 hover:text-white rounded-full transition-all disabled:opacity-50 disabled:hover:bg-nomad-700 ${compact ? 'px-4 text-xs' : 'px-6 text-sm font-medium'}`}
          >
            {isLoading ? (
